...
IGL_INLINE void igl::unproject (//我说你能不能给加一个类型别名...命名空间一层套一层
		const Eigen::MatrixBase<Derivedwin>& win,//用户视角的点
		const Eigen::MatrixBase<Derivedmodel>& model,//模型？什么用
		const Eigen::MatrixBase<Derivedproj>& proj,//投影矩阵
		const Eigen::MatrixBase<Derivedviewport>& viewport,//OpenGL格式的视口
		Eigen::PlainObjectBase<Derivedscene>& scene//由Embree构造的“ 边界体积层次”，
) {
	if (win.cols () != 3) {//空间点必须为3列
		assert (win.rows () == 3);
		// needless transposes
